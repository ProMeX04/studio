...
export default function Home() {
	...
	const handleGenerate = useCallback(
		async (
			...
			try {
				...
				const flashcardBatches = Math.ceil(
					Math.max(0, flashcardsNeeded) / BATCH_SIZE
				)
				const quizBatches = Math.ceil(
					Math.max(0, quizNeeded) / BATCH_SIZE
				)
				...
				for (let i = 0; i < totalBatches; i++) {
					const numToGenerateFlashcards = Math.min(
						BATCH_SIZE,
						flashcardMax - currentFlashcards.cards.length
					)
					const numToGenerateQuiz = Math.min(
						BATCH_SIZE,
						quizMax - currentQuiz.questions.length
					)
				...
				// After all batches generated, update state once to avoid UI jumps
				setFlashcardSet({ ...currentFlashcards })
				setQuizSet({ ...currentQuiz })
			} catch (error) {
				console.error(error)
	...
	const loadInitialData = useCallback(async () => {
		...
		const quizStateData = (await db.get(
			"data",
			"quizState"
		)) as AppData<QuizState>
	...
	const handleViewChange = useCallback(
		async (newView: "flashcards" | "quiz") => {
			// Chỉ cập nhật khi giá trị thực sự thay đổi để tránh render lặp vô hạn
			if (view === newView) return

			setView(newView)

			// Lưu thay đổi vào IndexedDB
			const db = await getDb()
