...
export function QuickLinks({ hasBackground }: { hasBackground: boolean }) {
  ...
  const getIconForLink = (link: QuickLink) => {
    ...
    if (lowerUrl.includes('gmail.com') || lowerUrl.includes('mail.google.com')) {
        return (
          <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6">
            <title>Gmail</title>
            <path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.71L12 16.64l-6.545-4.93v9.292H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.908 1.528-1.145C21.69 2.28 24 3.434 24 5.457z" fill="#EA4335"/>
          </svg>
        )
    }
    if (lowerUrl.includes('firebase.google.com')) {
        return (
          <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6">
            ...
            <path d="M4.213 15.548 3.03 5.41a.208.208 0 0 1 .25-.233l6.572 1.571a.208.208 0 0 1 .157.304l-3.32 5.419 2.286-1.32a.208.208 0 0 1 .26.039l2.747 3.42a.208.208 0 0 1-.165.341l-6.93-.456a.208.208 0 0 1-.19-.275zm15.424-7.235-8.54 15.65a.208.208 0 0 1-.366.01L7.1 18.067l7.55 4.36a.208.208 0 0 0 .285-.173L20.2 3.992a.208.208 0 0 0-.363-.279zM3.44.331.024 12.83a.208.208 0 0 0 .278.221L14.47 7.57a.208.208 0 0 0 .088-.35L3.841.34a.208.208 0 0 0-.401-.009z" fill="#FFCA28"/>
          </svg>
        )
    }
    try {
      const { hostname } = new URL(link.url);
      return <img src={`https://www.google.com/s2/favicons?domain=${hostname}&sz=32`} alt={`${link.name} favicon`} className="h-6 w-6"/>
    } catch (e) {
      return <img src={`https://www.google.com/s2/favicons?domain=example.com&sz=32`} alt={`${link.name} favicon`} className="h-6 w-6"/>
    }
  };
    ...
    <div className="flex justify-center items-center gap-2 md:gap-4 p-3 rounded-lg w-full">
      {links.map((link) => (
        <div key={link.id} className="group relative">
            ...
            className={cn(
                "flex flex-col items-center gap-2 p-2 rounded-lg transition-colors w-24",
                hasBackground ? 'hover:bg-white/10' : 'hover:bg-secondary'
            )}
            >
            <div className={cn(
                "p-3 rounded-full",
                 hasBackground ? 'bg-black/20' : 'bg-secondary'
            )}>
                {getIconForLink(link)}
            </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogTrigger asChild>
            <div className={cn(
                "flex flex-col items-center gap-2 p-2 rounded-lg transition-colors w-24 cursor-pointer",
                hasBackground ? 'hover:bg-white/10' : 'hover:bg-secondary'
